...
class AboutIteration < Neo::Koan
  ...
  def test_each_is_a_method_on_arrays
    assert_equal true, [].methods.include?(as_name(:each)) # [].methods return an array of methods, so we need to check if each is included in that array
  end

  def test_iterating_with_each
    array = [1, 2, 3]
    sum = 0
    array.each do |item| # |item| means: "take each element and call it item inside this block"
      sum += item
    end
    assert_equal 6, sum
  end
  ...
  def test_inject_will_blow_your_mind
    ...
    # Grabs all the value from the array and apply the opperations inside the bracket with a starting number in inject's
    # parameter
  end
